...
_LLM_PHASE2_SYSTEM = (
    ...
    "Find entity names that MIGHT refer to the same real-world concept. "
    "Be LIBERAL — false positives are acceptable; they will be filtered in the next step."
)

_LLM_PHASE2_TEMPLATE = """\
Below are all entity names in the knowledge graph.
Identify groups of names that MIGHT be duplicates (same concept, different \
spelling / language / casing).
...
- Skip obviously different items — numbered codes are NEVER duplicates \
(CFD 1 ≠ CFD 2, BR 2 ≠ BR 3, Step 1 ≠ Step 2).

Entity names:
{entity_list}

Respond ONLY with a JSON array. Output [] if no candidates found.
[
...
async def _call_llm_phase2_candidates(
    entity_names: list[str],
    env_path: str,
    chunk_size: int = 1000,  # entities per chunk
) -> list[dict]:
    """
    Phase 2 — Liberal scan (with chunking).
    Send entity names in chunks to LLM; returns candidate duplicate groups.
    Each item: {"members": [...], "reason": "..."}.
    """
    try:
    ...
    print(f"    Processing {len(entity_names)} entities in {len(chunks)} chunk(s) of ~{chunk_size} each")
    
    all_results = []
    
    for chunk_idx, entity_chunk in enumerate(chunks, 1):
        entity_list = "\n".join(f'{i + 1}. "{n}"' for i, n in enumerate(entity_chunk))
        user_prompt = _LLM_PHASE2_TEMPLATE.format(entity_list=entity_list)
        print(f"    Chunk {chunk_idx}/{len(chunks)}: {len(user_prompt):,} chars  |  model: {model}")

        try:
            resp = await client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": _LLM_PHASE2_SYSTEM},
            ...
            print(f"  [!] Phase 2 Chunk {chunk_idx} LLM call failed: {e}")
            continue

        data = _parse_llm_json(raw, f"Phase 2 Chunk {chunk_idx}")
        ...
        print(f"    Chunk {chunk_idx} found {len(chunk_results)} candidate groups")
        all_results.extend(chunk_results)
    
    print(f"    Total candidate groups from all chunks: {len(all_results)}")
    return all_results
...
async def run_llm_dedup(
    ...
    3-phase LLM deduplication pipeline:
      ...
      Phase 2 (LLM liberal): scan all entity names → candidate groups
      Phase 3 (LLM strict) : confirm each candidate with full descriptions
    """
    dry_run = not apply
    print(f"\n{'=' * 62}")
    ...
    print(f"  Confidence: >= {threshold:.2f}  (Phase 3 merge threshold)")
    print(f"  Env file  : {env_path}")
    print(f"{'=' * 62}\n")
    ...
    try:
        ...
        # ── Phase 2: LLM liberal scan ─────────────────────────────────────────
        print("  [Phase 2] Liberal scan — LLM identifies candidate groups...")
        raw_candidates = await _call_llm_phase2_candidates(semantic_names, env_path)

        # Validate: discard hallucinated names
        name_set = set(semantic_names)
        valid_candidates: list[dict] = []
        hallucinated = 0
        for c in raw_candidates:
            known = [m for m in c["members"] if m in name_set]
            hallucinated += len(c["members"]) - len(known)
            if len(known) >= 2:
                valid_candidates.append({"members": known, "reason": c["reason"]})
        if hallucinated:
            print(f"  [!] {hallucinated} hallucinated name(s) discarded from Phase 2 output")

        llm_merges: list[dict] = []
        if not valid_candidates:
            print("  Phase 2: no candidates found.\n")
        else:
            print(f"  Phase 2: {len(valid_candidates)} candidate group(s):")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Similarity threshold for --detect-similar; confidence threshold for --llm-dedup Phase 3 (default: 0.85).",
    )
    parser.add_argument(
