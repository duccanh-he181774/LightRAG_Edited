...
def _build_llm_dedup_prompt(clusters: list[list[dict]]) -> str:
    """Build LLM prompt from similarity clusters. Each dict has entity_name and description."""
    groups_block_parts = []
    for i, cluster in enumerate(clusters, 1):
        ...
        for e in cluster:
            ...
            sim = e.get("max_sim")
            sim_str = f"  (max similarity: {sim:.3f})" if sim is not None else ""
...
async def _call_llm_for_dedup(
    ...
    Uses LLM_BINDING_API_KEY / LLM_BINDING_HOST / LLM_MODEL from env.
    ...
    groups_block = _build_llm_dedup_prompt(clusters)
    ...
    for item in decisions:
        ...
        if merge and isinstance(merge, dict):
            ...
            if canonical and aliases:
                merges.append({"canonical": canonical, "aliases": aliases})
    return merges
...
async def run_llm_dedup(
    ...
    try:
        ...
        # Step 1: Get embeddings — text-dup aliases excluded so every cluster member
        # has a unique normalised key (no within-cluster text dups possible).
        entities_all = await fetch_entities_with_vectors(conn, workspace, entity_table)
        entities = [e for e in entities_all if e["entity_name"] not in text_dup_aliases]
        print(f"  Entities with vectors : {len(entities_all)}  ({len(entities)} after text-dup filter)")
        if len(entities) < 2:
            print("  Not enough entities to compare.")
            return
